...
export function DoctorDashboardScreen() {
  ...
  // Optimistic delete + live status updates.
  useRealtimeEvent((event, payload) => {
    ...
    if (event === "appointment.updated" && payload?.id) {
      const id = Number(payload.id);
      const status = payload.status as Appointment["status"] | undefined;
      if (status) {
  ...
  async function handleAction(id: number, action: string) {
    ...
    // Optimistic: update list immediately; roll back on failure.
    if (action === "delete") {
      setAppointments((prev) => prev.filter((a) => a.id !== id));
    } else if (optimistic) {
      setAppointments((prev) =>
        prev.map((a) => (a.id === id ? { ...a, status: optimistic } : a))
      );
    }
