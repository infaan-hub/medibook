...
export function NotificationsScreen() {
  ...
  // Optimistic mark-read from other tabs / server echoes.
  useRealtimeEvent((event, payload) => {
    if (event === "notification.updated" && payload?.id) {
    ...
    if (event === "notification.created" && payload?.id) {
      const id = Number(payload.id);
      setNotifications((prev) => {
        if (prev.some((n) => n.id === id)) return prev;
        // Prepend a minimal row; full refresh reconciles via useRealtimeSync.
        const row = {
          id,
          notification_type: String(payload.notification_type ?? "system"),
          title: String(payload.title ?? "MediBook update"),
          message: String(payload.message ?? ""),
          related_appointment: payload.related_appointment != null ? Number(payload.related_appointment) : null,
          is_read: payload.is_read === true,
          created_at: String(payload.created_at ?? new Date().toISOString()),
        };
        return [row as never, ...prev];
      });
    }
  });
