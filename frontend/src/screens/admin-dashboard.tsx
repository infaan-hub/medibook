...
export function AdminCreateUserScreen() {
  ...
  return <div className="admin-workspace admin-form-page"><AdminHeader title="Add user" description="Create a verified patient or staff account." action={<Link to="/admin/users" className="admin-outline-button">Back to users</Link>} /><Card className="admin-form-card"><form onSubmit={submit}><div className="admin-form-grid"><TextField id="admin-first-name" label="First name" name="first_name" value={form.first_name} onChange={(e) => update("first_name", e.target.value)} required /><TextField id="admin-last-name" label="Last name" name="last_name" value={form.last_name} onChange={(e) => update("last_name", e.target.value)} required /><TextField id="admin-username" label="Username" name="username" value={form.username} onChange={(e) => update("username", e.target.value)} required /><TextField id="admin-email" label="Email" name="email" type="email" value={form.email} onChange={(e) => update("email", e.target.value)} required /><TextField id="admin-password" label="Temporary password" name="password" type="password" value={form.password} onChange={(e) => update("password", e.target.value)} required /><TextField id="admin-phone" label="Phone" name="phone" value={form.phone} onChange={(e) => update("phone", e.target.value)} /><label className="admin-field"><span>Account role</span><select value={form.role} onChange={(event) => update("role", event.target.value)}><option value="patient">Patient</option><option value="doctor">Doctor</option></select></label></div><div className="admin-form-actions"><Link to="/admin/users" className="admin-outline-button">Cancel</Link><button className="admin-primary-button" disabled={saving} type="submit"><UserPlus size={16} /> {saving ? "Creating..." : "Create user"}</button></div></form></Card></div>;
}
